...
describe("graph", function () {
  ...
  it("connects nodes", function () {
    ...
    assert(
      node2.getIn("position").input,
      node1.getOut("position"),
      "N2 Position is wired up (in)"
    );
    assert(
      node2.getIn("alpha").input,
      node1.getOut("alpha"),
      "N2 Alpha is wired up (in)"
    );
    assert(
      node2.getIn("color").input,
      node1.getOut("color"),
      "N2 Color is wired up (in)"
    );
    assert(
      node2.getIn("matrix").input,
      node1.getOut("matrix"),
      "N2 Matrix is wired up (in)"
    );

    assert(
      node1.getOut("position").output.length,
      1,
      "N1 Position has one connection (out)"
    );
    assert(
      node1.getOut("alpha").output.length,
      1,
      "N1 Alpha has one connection (out)"
    );
    assert(
      node1.getOut("color").output.length,
      1,
      "N1 Color has one connection (out)"
    );
    assert(
      node1.getOut("matrix").output.length,
      1,
      "N1 Matrix has one connection (out)"
    );

    assert(
      node2.getIn("position"),
      node1.getOut("position").output[0],
      "N1 Position is wired up (out)"
    );
    assert(
      node2.getIn("alpha"),
      node1.getOut("alpha").output[0],
      "N1 Alpha is wired up (out)"
    );
    assert(
      node2.getIn("color"),
      node1.getOut("color").output[0],
      "N1 Color is wired up (out)"
    );
    assert(
      node2.getIn("matrix"),
      node1.getOut("matrix").output[0],
      "M1 Matrix is wired up (out)"
    );

    // Verify other inputs/outputs are untouched
    assert(
      node2.getOut("matrix").output.length,
      0,
      "N2 Matrix is ignored (out)"
    );
    ...
    assert(
      node1.getOut("position").output.length,
      0,
      "N1 Position out is disconnected"
    );
    assert(
      node2.getIn("position").input,
      null,
      "N2 Position in is disconnected"
    );
    ...
    assert(
      node2.getIn("alpha").input,
      node1.getOut("alpha"),
      "N2 Alpha is still wired up (in)"
    );
    assert(
      node2.getIn("matrix").input,
      node1.getOut("matrix"),
      "N2 Matrix is still wired up (in)"
    );

    assert(
      !!node1.getOut("alpha").output.length,
      true,
      "N1 Alpha has at least one connection (out)"
    );
    assert(
      node1.getOut("matrix").output.length,
      1,
    ...
    assert(
      node2.getIn("alpha"),
      node1.getOut("alpha").output[0],
      "N1 Alpha is still wired up (out)"
    );
    assert(
      node2.getIn("matrix"),
      node1.getOut("matrix").output[0],
      "N1 Matrix is still wired up (out)"
    );

    // Verify that third node is connected.
    assert(
      node3.getIn("alpha").input,
      node1.getOut("alpha"),
      "N3 Alpha is wired up (in)"
    );
    assert(
      node3.getIn("color").input,
      node1.getOut("color"),
      "N3 Color is wired up (in)"
    );
    assert(
      node3.getIn("position").input,
      node1.getOut("position"),
      "N3 Position is wired up (in)"
    );

    assert(
      node1.getOut("alpha").output.length,
      2,
      "N1 Alpha has two connections (out)"
    );
    assert(
      node1.getOut("color").output.length,
      1,
    ...
    assert(
      node1.getOut("matrix").output.length,
      0,
      "N1 Matrix output is not connected (out)"
    );
    assert(
      node1.getOut("color"),
      undefined,
      "N1 Color output has disappeared (out)"
    );
    ...
    assert(
      node3.getIn("alpha").input,
      node1.getOut("alpha"),
      "N3 Alpha is still wired up (in)"
    );
    assert(
      node3.getIn("position").input,
      node1.getOut("position"),
      "N3 Position is still wired up (in)"
    );
    ...
    assert(
      node1.getOut("position").output.length,
      1,
      "N1 Position has 1 connection (out)"
    );
    ...
    assert(
      node3.getIn("position"),
      node1.getOut("position").output[0],
      "N1 Position is still wired up (out)"
    );
    ...
    assert(
      inputs.indexOf(node1.getIn("color")) >= 0,
      true,
      "Graph has N1 color input"
    );
    assert(
      inputs.indexOf(node2.getIn("color")) >= 0,
      true,
      "Graph has N2 color input"
    );
    assert(
      inputs.indexOf(node3.getIn("color")) >= 0,
      true,
      "Graph has N3 color input"
    );
    assert(
      inputs.indexOf(node2.getIn("position")) >= 0,
      true,
      "Graph has N2 position input"
    );
    assert(
      inputs.indexOf(node2.getIn("matrix")) >= 0,
      true,
      "Graph has N2 matrix input"
    );
    assert(inputs.length, 5, "Graph has 5 inputs");

    const outputs = graph.outputs();
    assert(
      outputs.indexOf(node1.getOut("matrix")) >= 0,
      true,
      "Graph has N1 matrix output"
    );
    assert(
      outputs.indexOf(node2.getOut("matrix")) >= 0,
      true,
      "Graph has N2 matrix output"
    );
    assert(
      outputs.indexOf(node3.getOut("matrix")) >= 0,
      true,
      "Graph has N3 matrix output"
    );
